...
const ProfileTabs = () => {
  ...
  const { selectedTab, tabDropdownOpen } = useSelector(
    (state) => state.profile
  );
    ...
    <>
      <div className="mx-auto w-[85%]">
        <ul className="max-md:hidden grid grid-flow-col text-center text-slate-600 bg-slate-200 rounded-full p-1 space-x-5">
          <li>
            <button
              className={`${
                selectedTab === 0
                  ? "bg-white shadow text-indigo-900 hover:bg-opacity-100"
                  : "hover:bg-opacity-50"
              } w-full flex justify-center py-4  hover:bg-slate-50 rounded-full `}
            ...
            <button
              className={`${
                selectedTab === 2
                  ? "bg-white shadow text-indigo-900 hover:bg-opacity-100"
                  : ""
              } flex justify-center py-4 hover:bg-opacity-50 hover:bg-slate-50 rounded-full w-full `}
        ...
        <div className="relative flex items-center md:hidden h-24 w-24 mx-auto bg-slate-300 rounded-full shadow-2xl mt-6 cursor-pointer justify-between">
          <LuSettings
            className="mx-auto scale-[3.0] bg-slate-300  rounded-full w-6 h-6"
            onClick={() => dispatch(toggleTabDropdown())}
          />
          <div
            className={` ${
              tabDropdownOpen ? "" : "hidden"
            } absolute top-[212px] left-[50%] transform -translate-x-1/2 -translate-y-1/2 bg-[#2d2e2f] border-[#404344] border-2 mt-42 rounded-xl `}
          >
            ...
            <ul className="py-2 w-[70vw]" aria-labelledby="user-menu-button">
              <li>
                <button
                  className="w-full block px-4 py-3  text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white"
                  onClick={() => handleTabSelectionMobile(0)}
                >
                  Appointments
                </button>
              </li>
              <li>
                <button
                  className="w-full block px-4 py-3  text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white"
                  onClick={() => handleTabSelectionMobile(1)}
                >
              ...
              <li>
                <button
                  className="w-full block px-4 py-3  text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white"
                  onClick={() => handleTabSelectionMobile(2)}
                >
                  Settings
                </button>
              </li>
              <li>
                <button
                  className="w-full block px-4 py-3 text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white"
                  onClick={() => handleTabSelectionMobile(3)}
                >
                  Profile
                </button>
              </li>
