...
if (document && typeof document === "object"){
    const currentOrigin: string = document!.querySelector("#uniqueDonationScript")!.getAttribute("src")!.split("?")[1]
...
const desktopIframe: HTMLElement = document.createElement("iframe");
const mobileIframe: HTMLElement = document.createElement("iframe");
const desktopToggler: HTMLElement = document.createElement("div")
const mobileToggler: HTMLElement = document.createElement("div")
const icon: HTMLElement = document.createElement("img")
const iconMobile: HTMLElement = document.createElement("img")
const closeIcon: HTMLElement = document.createElement("img")
const closeIconMobile: HTMLElement = document.createElement("img")
...
function visibleFunction<t>(display: t): string {
    if(display=="block"){
        desktopToggler.style.backgroundColor = "lightblue"
        mobileToggler.style.backgroundColor = "lightblue"
        return "none"
...
desktopToggler.addEventListener("click", ()=>{
    if(window.innerWidth > 450){
        desktopIframe.style.display = visibleFunction<string>(desktopIframe.style.display)
    }else{
        mobileIframe.style.display = visibleFunction<string>(mobileIframe.style.display)
    }
